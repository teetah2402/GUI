...
// File NAME : C:\FLOWORK\flowork-gui\template\web\src\views\desktop\AppStoreDesktop.vue
//#######################################################################
      ...
      <div class="center-container px-safe pb-nav">
        ...
        <div v-else-if="filteredApps.length === 0" class="state-container">
          ...
          <p class="text-body-1 text-muted mt-4 font-weight-medium">
            {{ currentFilter === 'favorites' ? 'No favorites yet.' : (currentFilter === 'local' ? 'Tidak ada aplikasi offline yang ditemukan di folder apps/ PC Anda.' : 'No apps found matching your search.') }}
          </p>
...
import { useSocketStore } from '@/store/socket'; // Import Socket
...
const resolveImage = (app) => {
  let img = app.coverUrl || app.seo?.og_image;

  if (app.is_local) {
      if (img && img.startsWith('http')) return img;
      return img || `http://127.0.0.1:5000/local-apps/${app.id}/cover_mobile.webp`;
  }

  if (img) {
    img = img.replace(/_id\.webp|_en\.webp|_m_id\.webp|_m_en\.webp/gi, `_mobile.webp`);
    img = img.replace(/cover\.webp/gi, `cover_mobile.webp`);
  } else {
    img = `cover_mobile.webp`;
  }
...
const categories = computed(() => {
  ...
  appStore.installedApps.forEach(app => {
      if (app.desktop !== 'no' && app.category) cats.add(app.category);
  });
  return Array.from(cats);
});

// [TAMBAHAN KODE] Membaca data langsung dari memori appStore layaknya Node
...
const filteredApps = computed(() => {
  ...
  apps = apps.filter(app => app.desktop !== 'no');
  ...
  if (searchQuery.value) {
    ...
    apps = apps.filter(app => {
      const mName = app.name ? app.name.toLowerCase().includes(q) : false;
      const mDesc = app.description ? app.description.toLowerCase().includes(q) : false;
      const mKey = app.seo?.keywords ? app.seo.keywords.some(k => k.toLowerCase().includes(q)) : false;
      return mName || mDesc || mKey;
    });
  }
...
onMounted(() => {
  ...
  // [KUNCI PERBAIKAN] Panggil trigger connect saat halaman Store dimuat
  socketStore.connect();
});
...
.center-container { width: 100%; max-width: 1280px; margin: 0 auto; position: relative; padding-top: 80px; }
...
@media (min-width: 600px) { .app-grid { grid-template-columns: repeat(3, 1fr); gap: 20px; } }
@media (min-width: 960px) { .app-grid { grid-template-columns: repeat(4, 1fr); gap: 24px; } }
@media (min-width: 1280px) { .app-grid { grid-template-columns: repeat(5, 1fr); gap: 24px; } }
...
.action-overlay { position: absolute; top: 10px; left: 10px; display: flex; opacity: 0; transform: translateY(-5px); transition: transform 0.3s ease, opacity 0.3s ease; }
.app-card:hover .action-overlay { opacity: 1; transform: translateY(0); }
@media (max-width: 768px) { .action-overlay { opacity: 1; transform: none; } }
