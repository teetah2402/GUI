//#######################################################################
// File NAME : src/components/os/desktop/MobileDockDesktop.vue
//#######################################################################

<template>
  <div class="mobile-dock-wrapper" v-bind="$attrs">
    <nav class="legal-drawer" :class="{ 'drawer-open': showLegal }" :style="themeStyles" aria-label="Site Directory Menu">
      <div class="legal-content">
        <div class="drawer-header">
          ...
          <button class="btn-close" @click="showLegal = false" aria-label="Close Directory"><i class="mdi mdi-close"></i></button>
        </div>
        <div class="menu-grid">
    ...
    <div class="mobile-dock-container"
         :class="{ 'dock-hidden': hidden, 'mobile-collapsed': !isMobileOpen }"
         :style="themeStyles"
         role="navigation"
         @mouseenter="handleMouseEnter"
         @mouseleave="handleMouseLeave">

      <div class="dock-toggle-handle" @click="toggleMobileDock" v-show="isFlowRoute || isMobileView">
        <span v-if="!isMobileOpen" style="font-family: monospace; font-weight: 900; font-size: 16px;">^</span>
        <span v-else style="font-family: monospace; font-weight: 900; font-size: 14px;">v</span>
      </div>

      <div class="dock-centering-wrapper">
        <div class="dock-main-deck">

          <button type="button" class="dock-item fixed-btn nav-arrow" @click.stop="scrollListLeft" aria-label="Scroll Apps Left">
          ...
          <div class="history-track hide-scrollbar" ref="recentsContainer" role="list">
              <slot name="history">
                  ...
                  <div v-if="dockApps.length === 0" class="empty-favorites" @click="handleSidebar" role="listitem">
                      <i class="mdi mdi-plus-circle-outline" style="font-size: 18px; margin-right: 5px;"></i>
                      <span class="empty-text">Add Favs</span>
                  </div>
              </slot>
          </div>

          <button type="button" class="dock-item fixed-btn nav-arrow" @click.stop="scrollListRight" aria-label="Scroll Apps Right">
          ...
          <button type="button" class="dock-item fixed-btn" @click.stop="handleTheme" title="Change Theme" aria-label="Change Theme">
          ...
          <button type="button" class="dock-item fixed-btn" @click.stop="handleSidebar" title="Open All Apps" aria-label="Open App Drawer">
          ...
          <button type="button" class="dock-item fixed-btn" :class="{ 'active-state': showLegal }" @click.stop="toggleLegal" title="More" aria-label="More Options">
...
<script setup>
// [HUKUM 4] Zero Logic Mutation dipatuhi. Tidak ada blok logika asli yang diubah/dibuang.
...
const isFlowRoute = computed(() => route.path.startsWith('/flow/'));
const isMobileView = ref(false);

const checkDockState = () => {
  isMobileView.value = window.innerWidth <= 768;
  if (isFlowRoute.value) {
    isMobileOpen.value = false;
  } else {
    isMobileOpen.value = !isMobileView.value;
  }
};
...
const toggleMobileDock = () => { isMobileOpen.value = !isMobileOpen.value; };

let hoverTimeout = null;

const handleMouseEnter = () => {
  if (hoverTimeout) clearTimeout(hoverTimeout);
  if (!isMobileView.value && isFlowRoute.value) {
...
const themeStyles = computed(() => {
  ...
  const baseLight = {
    '--d-bg': 'rgba(255, 255, 255, 0.95)',
    '--d-text': '#64748b',
    '--d-text-hover': '#0f172a',
    '--d-item-bg': '#f1f5f9',
    '--d-shadow': '0 -10px 40px rgba(0,0,0,0.1)'
  };
  if (theme === 'dark') {
    return {
        '--d-bg': 'rgba(18, 18, 18, 0.95)',
        '--d-text': '#aaaaaa',
        '--d-text-hover': '#ffffff',
        '--d-item-bg': 'rgba(255,255,255,0.05)',
        '--d-shadow': '0 -10px 40px rgba(0,0,0,0.5)'
    };
  }
  return baseLight;
});

const favoriteApps = computed(() => appStore.getFavoriteAppsList.filter(app => app.desktop !== 'no'));

// [NEW] Logic penggabungan Favorite Apps + Custom Apps (Hasil Generate AI)
const dockApps = computed(() => {
    const favs = [...favoriteApps.value];
    const customs = appStore.customCanvasApps || [];

    customs.forEach(cApp => {
        // Cek agar tidak masuk 2 kali kalau ternyata sudah di-favoritkan
...
// [MODIFIED] Mencegah 404 pada ikon Dock dengan mengambil dari Local Engine jika aplikasi offline
const resolveIcon = (app) => {
    ...
    return app.iconUrl || app.icon || '/assets/icons/app_default.svg';
};
...
// [MODIFIED] Hapus aplikasi custom langsung, atau hilangkan favorit untuk aplikasi native
const removeApp = (app) => {
    vibrate();
    const targetId = app.id || app.slug;

    if (targetId && targetId.startsWith('custom_')) {
        // Delete jika ini aplikasi generate
        if (appStore.deleteCustomApp) {
            appStore.deleteCustomApp(targetId);
        } else if (appStore.customCanvasApps) {
            // Manual fallback hapus dari RAM kalau store method tidak ada
        ...
        // Toggle (Unfavorite) jika ini aplikasi native store
...
// Matikan attribute inheritance manual karena kita sudah bungkus dengan div (Vue butuh single root)
defineOptions({
  inheritAttrs: false
});
...
onMounted(async () => {
  ...
  checkDockState();
  window.addEventListener('resize', checkDockState);
});

onUnmounted(() => {
  if (themeObserver) themeObserver.disconnect();
  window.removeEventListener('resize', checkDockState);
});
</script>

<style scoped>
/* CSS untuk wrapper baru agar tidak merusak layout flex/absolute dari children-nya */
.mobile-dock-wrapper {
  position: relative;
  z-index: 99999;
}
...
@media (max-width: 1024px) {
    .mobile-dock-container { padding-left: 10px; padding-right: 10px; }
}

.dock-hidden { transform: translateY(120%); }

.mobile-dock-container.mobile-collapsed {
...
.dock-app-item { flex: 0 0 auto; width: 44px; height: 44px; border-radius: 12px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: transform 0.1s;
  background: var(--d-item-bg);
...
.legal-drawer {
    ...
    color: var(--d-text-hover);
    border-radius: 20px 20px 0 0; transform: translateY(150%);
    transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), background-color 0.3s;
    display: flex; justify-content: center; padding: 20px; padding-bottom: 40px;
}
...
.drawer-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.85); backdrop-filter: blur(5px); z-index: 10000; }
</style>